...
config="/path/to/${dir}/to-map/${input}"
...
bwa mem \
    /path/to/data/${ref}\
    /path/to/${dir}/to-map/${name}.R1.fastp.fq.gz \
    /path/to/${dir}/to-map/${name}.R2.fastp.fq.gz \
    -t 15|
samtools view -hbS - -o /path/to/${dir}/bams/${name}.PR.bam -@ 15
